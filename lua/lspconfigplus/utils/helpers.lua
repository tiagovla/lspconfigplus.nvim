...
-- path of lsp server
function U.install_server_path(server_name)
    return config.install_path .. "servers/" .. server_name
end
function U.install_formatter_path(formatter_name)
    return config.install_path .. "formatters/" .. formatter_name
end
function U.install_linter_path(linter_name)
    return config.install_path .. "linters/" .. linter_name
end
...
function U.available_servers(servers)
...
function U.installed_servers(servers)
    return vim.tbl_filter(function(key)
        return U.is_server_installed(key)
    end, U.available_servers(servers))
end

-- table with non installed servers
function U.not_installed_servers(servers)
    return vim.tbl_filter(function(key)
        return not U.is_server_installed(key)
    end, U.available_servers(
        servers
    ))
end
