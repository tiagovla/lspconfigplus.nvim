...
local filetype_cmds = {
    ...
    "syntax clear", "syn match lspconfigplusWorking /^ " .. config.working_sym .. "/",
...
function Display:task_start(server, message)
    ...
    return vim.schedule_wrap(function() fn(server, message) end)
end
...
function Display:task_failed(server, message)
    local function fn(server_, message_)
        ...
        self:set_lines(line[1], line[1] + 1,
                       {fmt(" %s %s: %s", config.error_sym, server_, message_)})
        api.nvim_buf_del_extmark(self.buf, self.ns, self.marks[server_])
        self.marks[server_] = nil
        self:decrement_headline_count()
    end
...
function Display:task_update(server, message)
    local function fn(server_, message_)
        if not self:valid_display() then return end
        if not self.marks[server_] then return end
        local line, _ = utils.get_extmark_by_id(self.buf, self.ns, self.marks[server_])
        self:set_lines(line[1], line[1] + 1,
                       {fmt(" %s %s: %s", config.working_sym, server_, message_)})
        utils.set_extmark(self.buf, self.ns, self.marks[server_], line[1], 0)
    end
    return vim.schedule_wrap(fn(server, message))
end

function Display:quit() vim.fn.execute("q!", "silent") end

function Display:toggle_info() print("showing more info") end

function Display:make_header()
    local width = api.nvim_win_get_width(0)
    api.nvim_buf_set_lines(self.buf, 0, 1, true, {
        string.rep(" ", 3) .. "lspconfig+", " " .. string.rep("-", width - 2) .. " ",
    })
end
...
function Display:setup_window()
    ...
    for _, c in ipairs(filetype_cmds) do vim.cmd(c) end
end
